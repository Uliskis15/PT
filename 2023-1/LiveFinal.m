...
ms=2;%Tasa de subida CDN
a=1;%Limite en y de las rectas
...
for idxc=1:length(C)
   ...
   for idxt=1:length(teta) 
      ...
      PT=zeros(1,C(idxc)+1);%Vector de poblaciones por ventana de la hiperventana
      ...
      tab=zeros(1,C(idxc)+1);%Vector para tasa de abandono
      tao_cw=zeros(1,C(idxc)+1);%Vector para tasa de descarga
      tao_mw=zeros(1,C(idxc)+1);%Vector para tasa de subida
      ...
      for iter=1:IT
          ...
          %Caso estado ~= (0,0,0,....,1)
             ...
             tao_cw(2:C(idxc)+1)=(cw-Pw)*HV(2:C(idxc)+1);%Tasa promedio de descarga en abundancia
    
             %Obtener rectas para escalar msi
             kv=C(idxc)/2;%Ventana de enmedio
             m1=-a/kv;
             m2=a/kv;
             for i=1:C(idxc)+1
                 if i<=kv
             ...
             if Si >= 2
                if desc>0
                   ttran(C(idxc))=desc;%Tasa de producción de la ventana C+1
                else
                   ttran(C(idxc))=inf;
                   tao_min(C(idxc)+1)=inf;
                end
             else
                ttran(C(idxc))=inf;
                tao_min(C(idxc)+1)=inf;
             ...
             for idxtao=1:C(idxc)+1%Cambir el vector a infinitos para descartar poblaciones en 0
                 if(tran(idxtao)==0)
                    TTran(idxtao)=inf;
                 else
