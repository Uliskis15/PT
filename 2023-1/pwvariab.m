...
P=[0.2 0.5 0.8];%Tasa de producción del video
...
IT=200000;%Número de iteraciones
...
for idxp=1:length(P)
    ...
    for idxc=1:length(C)
       ...
       for idxt=1:length(teta) 
          ...
          Pw=P(idxp)*cw;%Tasa de producción del video
...
%zticks([0:2:20])
%zlim([0 20])
legend([G1 G2 G3],{'P\omega=0.2C\omega','P\omega=0.5C\omega','P\omega=0.8C\omega'})
ylabel('\theta')
xlabel('C')
zlabel('x')
