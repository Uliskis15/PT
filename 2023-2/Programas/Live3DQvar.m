...
for idxq=1:length(Q)
    ...
    %              VEPw=min(TProd);% Obtener minimo de prod
                 ...
                 elseif Evfinal==VETao
                    ...
                    BW(1,:)=BW(1,:)+(tao_cw*VETao);
                    for ind=1:C(idxc)
                        if tao_cw(ind)>TAO_MW(ind)
                           BWP2P(1,ind)=BWP2P(1,ind)+(tao_mw(ind)*VETao);
                           BWSer(1,ind)=BWSer(1,ind)+(tao_serv(ind)*VETao);
                        else
                           m=min(tao_cw(ind),tao_mw(ind));
                           BWP2P(1,ind)=BWP2P(1,ind)+(m*VETao);
                           BWSer(1,ind)=BWSer(1,ind)+((tao_cw(ind)-m)*VETao);
                        end
                    end
                 end
