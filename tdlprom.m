      ...
      for iter=1:IT
          ...
          %Caso estado ~= (0,0,0,....,1)
              ...
              for i=1:C(idxc)
                  ...
                  tao_mw(i)=tao_mw(i)+(HV(C(idxc)+1)/sum(HV(1:C(idxc))))+(1/(1+2^(i)))*ms;
              end
         ...
         Tdl(idxc,idxt)=1/(cw+teta(idxt));
         %disp('------');
   end
   Tdl0(1,idxc)=1/(cw+teta0);
end
     
figure(1)
surf(C,teta,transpose(Tdl),'FaceAlpha',0.25)
xticks([12:4:36])
...
xlabel('C')
zlabel('Tiempo(s)')
title('Tiempo promedio de descarga por ventana')
...
ylabel('Tiempo(s)')
xlabel('C')
title('Tiempo promedio de descarga de la ventana 0')